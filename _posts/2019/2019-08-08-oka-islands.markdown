...
categories: blog
excerpt: "Islands of the Okavango wetlands"
tags:
  - Okavango
  - islands
...
The dryland region to the north west of the Thamalakane/Kunyere fault line (Figure 2), excluding the Okavango swamps and spanning both sides of the Panhandle entry channel, was interpolated using three combined datasets; GPS elevations from the gravity survey, beacon heights drawn from Botswana maps and heights from the Digital Chart of the World (DCW), table 3.
...
The region between the Kunyere and Thamalakane faults lines ("kunyere-trappa") (figure 2) was interpolated separately. The region is intersected by the rivers draining the distal arms of the Okavango swamps before they join the Thamalakane River (defined by the Thamalakane fault line). The "Kunyere-trappa" region was interpolated using GPS elevations from the GPS/gravity survey and trigonometric beacon heights from Botswana maps, table 5.

<figcaption>
Table 5. Elevation point data sources used for interpolating the region "kunyere-trappa" (see figure 1). All the data layers are projected to SUTM 34 using the WGS84 datum (**not** the Cape datum as in the scientific article, the data has been reprojected to a more standardized format). All layers are available for download [here](), or you can click the Layer name to download each layer.
</figcaption>
...
The last, central, region is the maximum flood area (see [this]() post) of the Okavango wetlands, and the region is called "oka-wetlands". To interpolate this region we used three different data sets; GPS elevations the gravity survey, beacons from Botswana maps and GPS points collected by University of Cape Town (UCT), table 6.

<figcaption>
Table 6. Elevation point data sources used for interpolating the region "NW-drylands" (see figure 1). All the data layers are projected to SUTM 34 using the WGS84 datum  All layers are available for download towards the end of this post, or you can click the Layer name to download each layer.
</figcaption>
...
The DEM for the larger Okavango region was created by combining the four regional DEMs and then smoothing the compositional DEM across areas with no pronounced faults (for details please see the [original article](http://doi.org/10.2113/1040243)). A few pronounced heights and troughs were added, and the elevation contour lines extracted. The final DEM, as a 50 mm raster and as contour lines are listed and available for download in table 8.

<figcaption>
Table 8. Spatial data layers available for the larger Okavango region. All the data layers are projected to SUTM 34 using the WGS84 datum. All layers are available for download towards the end of this post, or you can click the Layer name to download each layer.
</figcaption>
...
The Okavango Alluvial Fan (Figure 3), forms an almost perfectly shaped cone but where small variations potentially could reveal sedimentation, erosion or tectonic movements. To compare the modeled DEM with a theoretical cone, several spatial entities related to the Okavango Alluvial Fan where identified or created, table 9.

<figcaption>
Table 9. Spatial data layers associated with Okavango the Alluvial Fan. All the data layers are projected to SUTM 34 using the WGS84 datum. All layers are available for download towards the end of this post, or you can click the Layer name to download each layer.
</figcaption>

|  Layer name  | Layer content |  
|:---------------------|:----------------------|
| [alluvial-fan-apex_karttur_okaswamps_0_v2001](../../docs/dem/alluvialfan/alluvial-fan-apex_karttur_okaswamps_0_v2001.zip) | The apex (beginning, point from spreading out)<br> of the Okavango Alluvial Fan |
| [alluvial-fan_karttur_okaswamps_0_v2001](../../docs/dem/alluvialfan/alluvial-fan_karttur_okaswamps_0_v2001.zip) | The surface area of the Okavango Alluvial Fan |
| [alluvial-fan-isodist_karttur_okaswamps_0_fromapex](../../docs/dem/alluvialfan/alluvial-fan-isodist_karttur_okaswamps_0_fromapex.zip) | Iso-distance lines (25 km interval) from the apex |
| [alluvial-fan-cone-dem_karttur_okasvamps_0_v2001](../../docs/dem/alluvialfan/alluvial-fan-cone-dem_karttur_okasvamps_0_v2001.zip) | Idealized, perfectly cone shaped DEM<br> over the Okavango Alluvial Fan |
| [alluvial-fan-residual-dem_karttur_okasvamps_0_v2001](../../docs/dem/alluvialfan/alluvial-fan-residual-dem_karttur_okasvamps_0_v2001.zip) | elevation difference between the idealized and<br> the interpolated DEMs over the Okavango Alluvial Fan  |

### Lake Ngami Region

As noted above, a special DEM was created for Lake Ngami and its surroundings. The palaeo shorelines of Lake Ngami were omitted from the DEM that thus shows the base level of these shorelines. Extracting the base levels we could analyse the topography of the shorelines. The results indicate that the palaeo shorelines are tilted towards north-east, indicating tectonic movements (sagging) since palaeo Lake Ngami dried out. All the data for the Lake Ngami DEM are listed in table 10.
...
| [elev-pt-palaeo-shore_karttur_mkgadiridge_0_v2001](../../docs/dem/mkgadiridge/elev-pt-palaeo-shore_karttur_mkgadiridge_0_v2001.zip) | Elevation point data (best estimate and error)<br>  extracted from the generated DEM (GeoTiff)<br>for the base level of Makgadikgadi [id]: url "title"idge (palaeo-shoreline).  |
...
[**Elevation point data used as input for generating DEM**](../../docs/dem/input/).

[**Water surface elevation**](../../docs/dem/surfacewater/).

[**Geophysiological regions**](https://github.com/karttur/okavango/tree/gh-pages/docs/geophysiology/).
